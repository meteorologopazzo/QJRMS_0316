...
def bulk_ERA5(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free, frac_Ustar):
    ...
    return np.array([dh_dt, dq_dt, dth_dt, dU_dt, dV_dt])



def solve_bulk_point(t_start, t_end, dt_max, y0,
                     SST, D, q_free, th_free, ps, f, U_free, V_free, frac_Ustar):
    """Integrate one pixel. Returns final state or NaNs if diverged."""
    try:
        sol = solve_ivp(
            bulk_ERA5,
            (t_start, t_end),
            y0,
            max_step=dt_max,
            args=(SST, D, q_free, th_free, ps, f, U_free, V_free, frac_Ustar),
        )
        if not sol.success:
...
def solve_bulk_all(t_start, t_end, dt_max, y0_field,
                   SST, D, q_free, th_free, ps, f, U_free, V_free, frac_Ustar,
                   n_jobs=-1):
    """
    y0_field      : (nlat, nlon, n_states)
    forcing arrays: (nlat, nlon)
    """
    nlat, nlon, _ = y0_field.shape

    results = Parallel(n_jobs=n_jobs, verbose=1)(
        delayed(solve_bulk_point)(
            t_start, t_end, dt_max,
            y0_field[i, j, :],
            SST[i,j], D[i,j], q_free[i,j], th_free[i,j],
            ps[i,j],  f[i,j], U_free[i,j], V_free[i,j], frac_Ustar[i,j]
        )
        for i in range(nlat)
        for j in range(nlon)
    )

    return np.array(results).reshape(nlat, nlon, -1)
